...
```bash
serverless-rds-proxy-demo$ sam logs -n HelloEfsFunction --stack-name serverless-rds-proxy-demo --tail
```
...
```bash
serverless-rds-proxy-demo$ pip install -r tests/requirements.txt --user
# unit test
serverless-rds-proxy-demo$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
serverless-rds-proxy-demo$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name serverless-rds-proxy-demo
```
